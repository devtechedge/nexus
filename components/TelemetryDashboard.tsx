...
import { useEdgeTelemetry, AnomalyEvent } from "@/hooks/useEdgeTelemetry";
...
export const TelemetryDashboard: React.FC = () => {
  const { latestLog, logsHistory, isStreaming, user, setUser } = useTelemetryStore();
  ...
  const handleCopilotSend = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/copilot", {
        ...
        body: JSON.stringify({
          message: userMessage,
          metricsHistory: rollingMetricsAverages,
        }),
      });
  ...
  return (
    ...
    >
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-4" aria-live="polite">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between shadow-sm">
            ...
            <div className="flex flex-col items-end">
              ...
              <span className="text-[9px] font-mono text-slate-500 mt-1 uppercase">Stable</span>
            </div>
          </div>
              ...
              {anomalies.length === 0 ? (
                ...
                anomalies.map((an) => {
                  ...
                  return (
                    <div
                      key={an.id}
                      className={`p-2.5 rounded border flex flex-col gap-1 transition-all duration-300 ${
                        isCrit
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        {copilotOpen && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-[350px] sm:w-[420px] h-[500px] flex flex-col justify-between overflow-hidden mb-3 border-teal-500/20">
            {/* Header */}
